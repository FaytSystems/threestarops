...
function prepPriority(sel) { if (sel.priority) return num(sel.priority, 4); const flags = sel.flags || {}; if (flags.is_86 || flags.need_before_start) return 1; if (flags.watch_list || flags.expiring_soon) return 2; return 4; }
function normalizePrepTask(t) { const qty = num(t.qty ?? t.quantity, 0); const priority = num(t.priority, 4); return { ...t, qty, quantity: qty, priority, suggested_rank: num(t.suggested_rank, priority <= 2 ? priority : 3), need_by_label: t.need_by_label || t.due_at || t.due_date || t.prep_date || 'next shift', status: t.status || 'todo' }; }
function isOpenPrepTask(t) { return !['done', 'complete', 'completed', 'canceled', 'cancelled'].includes(lower(t.status)); }
async function stationBuild(env, row, body) { const tasks = []; for (const sel of (body.selections || [])) { const qty = num(sel.qty, 1); const priority = prepPriority(sel); tasks.push(await saveRecord(env, row, 'prep_task', { title: sel.name || sel.notes || `Prep ${sel.kind || 'item'}`, station: body.station || '', prep_date: body.prep_date || todayIso(), service_period: body.service_period || '', qty, quantity: qty, unit: sel.unit || 'each', status: 'todo', priority, suggested_rank: priority <= 2 ? priority : 3, flags: sel.flags || {}, notes: sel.notes || '', source: 'station_build' })); } return { ok: true, tasks, task_count: tasks.length }; }
async function managerPreplist(env, row) { const tasks = (await listRecords(env, row.team_id, 'prep_task')).map(normalizePrepTask).filter(isOpenPrepTask); const by_station = {}; tasks.forEach(t => { (by_station[t.station || 'Unassigned'] ||= []).push(t); }); return { tasks, by_station, needed_now: tasks.filter(t => Number(t.priority || 4) <= 2 || lower(t.flags?.is_86) === 'true' || lower(t.flags?.need_before_start) === 'true') }; }
function prepTemplateId(row, station) { return `prep_template_${row.team_id}_${lower(station || 'default').replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '') || 'default'}`; }
function stationMatches(item, station) { return !station || lower(item.station) === lower(station); }
async function stationBuildOptions(env, row, url) { const station = url.searchParams.get('station') || ''; const [products, recipes, dishes] = await Promise.all([listRecords(env, row.team_id, 'product'), listRecords(env, row.team_id, 'recipe'), listRecords(env, row.team_id, 'dish')]); const stationPlates = dishes.map(normalizeDish).filter(d => stationMatches(d, station)); const recipeNames = new Set(stationPlates.flatMap(d => [d.recipe_id, ...(d.recipes || []).map(r => r.id || r.recipe_id || r.component_id)].filter(Boolean).map(String))); const stationRecipes = recipes.map(normalizeRecipe).filter(r => stationMatches(r, station) || recipeNames.has(String(r.id))).map(r => ({ ...r, source_plates: stationPlates.filter(d => String(d.recipe_id) === String(r.id) || (d.recipes || []).some(x => String(x.id || x.recipe_id || x.component_id) === String(r.id))).map(d => d.name) })); const stationProducts = products.map(p => normalizeProduct(p)).filter(p => stationMatches(p, station)); return { station, plates: stationPlates, recipes: stationRecipes, products: stationProducts, selections: [] }; }
async function getStationTemplate(env, row, url) { const station = url.searchParams.get('station') || ''; const templates = await listRecords(env, row.team_id, 'prep_template'); const template = templates.find(t => lower(t.station) === lower(station)) || templates[0] || { station, selections: [] }; const [products, recipes] = await Promise.all([listRecords(env, row.team_id, 'product'), listRecords(env, row.team_id, 'recipe')]); const itemMap = new Map([...products.map(p => [`product:${p.id}`, normalizeProduct(p)]), ...recipes.map(r => [`recipe:${r.id}`, normalizeRecipe(r)])]); const items = (template.selections || []).map(sel => { const source = itemMap.get(`${sel.kind}:${sel.id}`) || {}; return { ...source, ...sel, id: sel.id, kind: sel.kind, name: source.name || sel.name || `Prep ${sel.kind || 'item'}`, default_qty: num(sel.qty, 1), unit: sel.unit || source.unit || source.portion_unit || 'each', station: template.station || source.station || station }; }); return { station: template.station || station, template, selections: template.selections || [], items }; }
async function saveStationTemplate(env, row, body) { const station = body.station || ''; const selections = Array.isArray(body.selections) ? body.selections : []; const template = await saveRecord(env, row, 'prep_template', { ...body, station, selections, saved_count: selections.length, updated_at: nowIso() }, prepTemplateId(row, station)); return { ok: true, saved: selections.length, saved_count: selections.length, template, items: selections }; }
...
function csvResponse(filename, csv) { return new Response(csv, { headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': `attachment; filename="${filename}"` } }); }
async function exportInventoryCsv(env, row) { const vendors = await listRecords(env, row.team_id, 'vendor'); const products = (await listRecords(env, row.team_id, 'product')).map(p => normalizeProduct(p, vendors)); const lines = [['Item','Category','Unit','Current Qty','Par','Reorder','Vendor','Station','Location'].map(csvCell).join(',')]; products.forEach(p => lines.push([p.name, p.category, p.unit, p.current_qty, p.par_qty, p.reorder_qty, p.vendor_name, p.station, p.stock_location || p.location].map(csvCell).join(','))); return csvResponse(`threestarops-inventory-${todayIso()}.csv`, lines.join('\n')); }
async function exportRecipesCsv(env, row) { const recipes = (await listRecords(env, row.team_id, 'recipe')).map(normalizeRecipe); const dishes = (await listRecords(env, row.team_id, 'dish')).map(normalizeDish); const lines = [['Type','Name','Station','Menu Price','Yield/Shift','Ingredients/Items'].map(csvCell).join(',')]; recipes.forEach(r => lines.push(['Recipe', r.name, r.station, r.menu_price, r.yield_servings, JSON.stringify(r.items?.length ? r.items : r.ingredients || [])].map(csvCell).join(','))); dishes.forEach(d => lines.push(['Plate', d.name, d.station, d.menu_price, d.shift, JSON.stringify(d.components || d.recipes || d.ingredients || [])].map(csvCell).join(','))); return csvResponse(`threestarops-recipes-plates-${todayIso()}.csv`, lines.join('\n')); }
...
async function handleApi(context) {
  ...
  if (method === 'GET' && path === 'export/inventory.csv') return exportInventoryCsv(env, activeRow);
  if (method === 'GET' && path === 'export/recipes.csv') return exportRecipesCsv(env, activeRow);
  ...
  if (method === 'GET' && path.startsWith('prep/station_template')) return json(await getStationTemplate(env, activeRow, url));
  if (method === 'POST' && path === 'prep/station_template') return json(await saveStationTemplate(env, activeRow, body));
  if (method === 'GET' && path.startsWith('prep/station_build')) return json(await stationBuildOptions(env, activeRow, url));
